...
>
> **Current version**: `jainkosh.rules/1.11.24`
>
...
- **`#fieldname` — ignored field (v1.11.23)**: a field name prefixed with `#` is still matched **positionally** (it consumes its value group / sub-separator slot and participates in mismatch detection exactly like a normal field) but is **dropped from the `resolved_fields` output**. Works on normal numeric fields, passthrough `<#field>`, and keyword-trigger `{…}#field` groups. Example: `#राजवार्तिकपुस्तक/अध्याय/राजवार्तिकसूत्र/पृष्ठ/पंक्ति` matches `1/9/2/600/12` and emits `अध्याय=9, राजवार्तिकसूत्र=2, पृष्ठ=600, पंक्ति=12` while the leading `राजवार्तिकपुस्तक=1` is consumed but omitted. Implemented via `FormatField.ignored` (set by `_split_ignored` in `parse_format_string`).
...
**Keyword-aware numeric split (v1.11.14)**: `numeric_clean_with_kw` is produced by `split_name_and_numeric_kw` (not the plain `split_name_and_numeric`). This helper also splits at `/keyword` boundaries that appear **before** the first bare digit in the text. This ensures a keyword that is part of a trigger group (e.g. "कलश" in "/ कलश 2") lands in the numeric portion rather than being absorbed into the shastra/teeka name, allowing `{कलश}कलश`-style formats to match. Regular (non-keyword-trigger) formats are unaffected because they use the keyword-stripped `numeric_clean` string.
...
**Compound-shastra gatha-field detection (v1.11.24)**: the gatha-entity match in `_try_split_multi_verse` is **suffix-aware**, not exact. For compound shastras the gatha-entity field is named in the shastra-grammar form (`तत्त्वार्थसूत्र` → `तत्त्वार्थसूत्रसूत्र`, `परमात्मप्रकाश` → `परमात्मप्रकाशगाथा`), so an exact match against `entity_keywords.gatha` (`गाथा`/`श्लोक`/`सूत्र`/…) missed it. The local `_is_gatha_field(name)` helper returns `True` when `name` is in `entity_keywords.gatha` **or** ends with any of those keywords, and is used by `_gatha_value`, the per-split field-name pick, and the strip filter. Before the fix, splitting a ranged compound reference (e.g. `तत्त्वार्थसूत्र/5/33-36`) (a) kept the base ref's entity value pinned for every split (`तत्त्वार्थसूत्रसूत्र=33` on all of 33–36) and (b) added a phantom `गाथा` field carrying the varying value. After the fix the entity field itself varies per marker (`तत्त्वार्थसूत्रसूत्र` → 33, 34, 35, 36) and no phantom `गाथा` is emitted — consistent with the Gatha NK (`तत्त्वार्थसूत्र:अध्याय:5:सूत्र:36`).
...
| `1.11.23` | **Ignored-field syntax `#fieldname` in format strings**: a format field prefixed with `#` is matched positionally (consumes its slot, participates in mismatch detection) but excluded from `resolved_fields`. Applies to normal numeric fields, passthrough `<#field>`, and keyword-trigger `{…}#field` groups. `FormatField.ignored` flag added; `_split_ignored` strips the marker in `parse_format_string`; `_assign_group`, `_assign_keyword_group`, and the passthrough branch of `resolve_fields` skip emission for ignored fields. Used in राजवार्तिक format 1 (`#राजवार्तिकपुस्तक/अध्याय/राजवार्तिकसूत्र/पृष्ठ/पंक्ति`) to consume the leading पुस्तक without surfacing it. |
...
| `1.11.24` | **Compound-shastra gatha-field detection in multi-verse split**: `_try_split_multi_verse` now matches the gatha-entity field by **suffix** (via local `_is_gatha_field`), not exact name. Compound shastras name their gatha-entity field in the shastra-grammar form (`तत्त्वार्थसूत्र` → `तत्त्वार्थसूत्रसूत्र`, `परमात्मप्रकाश` → `परमात्मप्रकाशगाथा`), which the old exact match against `entity_keywords.gatha` (`गाथा`/`श्लोक`/`सूत्र`/…) missed. Before the fix, splitting a ranged compound reference like `तत्त्वार्थसूत्र/5/33-36` (a) left the entity field pinned to the base value (`तत्त्वार्थसूत्रसूत्र=33` on all of 33–36) and (b) added a phantom `गाथा` field carrying the varying value. After the fix the entity field varies per marker (`तत्त्वार्थसूत्रसूत्र` → 33/34/35/36) and no phantom `गाथा` is emitted — consistent with the emitted Gatha NK (`तत्त्वार्थसूत्र:अध्याय:5:सूत्र:36`). Applied at `_gatha_value`, the per-split field-name pick, and the strip filter. |
...
| स्वभाव | `तत्त्वानुशासन/53` in a `sanskrit_text` block — type=publication, no `teeka_name` → was emitting `Gatha`, now emits `GathaTeeka("तत्त्वानुशासन:टीका:गाथा:टीका:53")` | `publication` + `sanskrit_text` rule (§12.2, v1.11.19): `<teeka>` defaults to `"टीका"` when absent. |
| स्वभाव | `धम्मो वत्थुसहावो।` citation as `prakrit_text` — type=publication, `कार्तिकेयानुप्रेक्षा/478` → emits `Gatha` (Prakrit original source, same as `prakrit_gatha`) | `publication` + `prakrit_text` → `Gatha` (§12.2, v1.11.19). |
