...
| `DefinitionModal` | `components/DefinitionModal.tsx` | Full-screen `@base-ui/react` dialog. Keyword path: all sections → `ModalBlock` per block; `<hr>` dividers between sections and between definitions. Topic path: extracts grouped by shastra name via `groupTopicExtractsByShastra` → each group rendered as a collapsible `ShastraAccordion` (chevron toggle, default expanded, count in parentheses, indented body). **Reference display per block:** all non-inline refs (`inline_reference: false`) with resolved fields are shown as badges on the left; if only inline refs exist, only the first qualifying one is shown. If additional matched refs were hidden (inline refs suppressed when non-inline exist, or extra inline refs beyond the first), a **`समान संदर्भ`** button (accent red, bold, rightmost) opens a 480px `Popover` (column layout) listing them. Left border colour: teal for Sanskrit/Prakrit, amber for teeka refs, sky-blue for shastra refs. Teeka badge label = `shastra_name, teeka_name`. Exports `getBlockBorderClass`, `formatRefSourceLabel`, `pickRefsToShow`, `pickHiddenRefs`, `groupTopicExtractsByShastra`, `ShastraGroup` (all pure / typed, tested). Closes on node selection change. |
