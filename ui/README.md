...
3. **Incremental expand** — `expanderPos` known, existing nodes remain, new children arrived. A snapshot is saved, then:
   - **Hierarchical** — children are placed in **wrapped rows** under the expander: `MAX_PER_ROW = 5`, `childColSpacing = HIER_NODE_SPACING * 0.85`, `childRowSpacing = HIER_LEVEL_HEIGHT * 0.75`. Each row centres on `expanderPos.x` (the last, possibly shorter, row centres on its own count). Large neighbourhoods cluster locally instead of stretching across the canvas.
...
After every branch, the committed positions of all `simNodes` are written back into `lastPositionsRef` for the next incremental step.
