...
| `PanelActionsMenu` | `components/PanelActionsMenu.tsx` | `'use client'`. `MoreHorizontal` (⋯) `Popover` trigger placed in each panel header. Two list items (`Tag` icon → topics, `Sparkles` icon → keywords) that call `useReaderActions().open()` with the panel's source nk. **Embedded in panels, not passed as a callback** — function props can't cross the server→client boundary, so `TabbedPanel` / `TeekaPanel` accept serialisable `showActions: boolean` + per-item `actionsSourceNk` / `actionsSourceLabel` fields and render the menu themselves from the currently-active tab item. |
