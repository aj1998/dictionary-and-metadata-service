...
| `BhaavarthPanel` | `components/BhaavarthPanel.tsx` | Bordered surface panel with an optional label and text body. Two rendering variants controlled by the `variant` prop: `'prose'` (default) — runs text through `teekaMarkdownToHtml` which wraps `\n\n`-separated paragraphs and `\n`-separated lines in `<p>` elements (correct for teeka/bhaavarth prose); `'verse'` — uses `whitespace-pre-wrap` directly, keeping doha/gatha line breaks tight without extra paragraph margins. Pass `variant="verse"` for all kalash Sanskrit/Hindi and secondary-gatha Prakrit content. |
