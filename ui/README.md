...
| `TopicTreeBrowser` | `components/TopicTreeBrowser.tsx` | `'use client'`. Horizontal multi-column topic navigator used on dictionary keyword and topic detail pages. Column 0 is seeded server-side (keyword page: `getKeywordTopics`; topic page: `getTopicNeighbors` filtered to inbound `PART_OF`). Clicking a row's label expands its inbound-`PART_OF` children into the next column via `getTopicNeighbors`. Each row shows an ASCII dotted path (e.g. `1`, `1.1`, `1.1.1`). On render, every visible item is probed once via `getEntityDetail('topic', nk)` (in-memory cache + in-flight dedupe); the probe drives three affordances: a small bordered-accent `BookOpen` pill (opens `DefinitionModal` with the topic's extracts) when `topicExtracts` is non-empty, a muted `›` `ChevronRight` when `stats.is_leaf === 0`, and disabling of the label-button expand action for confirmed leaves. Both affordances render together when applicable. |
