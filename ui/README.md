...
- **⚠️ Dismiss-on-scroll (do not remove):** because `top`/`left` are computed **once** from `getBoundingClientRect()` at click time and never recomputed, the fixed popover does **not** follow its anchor when the page scrolls — it stays glued to the viewport while the text moves out from under it (looks like the टिप्पणी "drifting" over unrelated prose). The fix is **not** live position tracking; the popover is dismissed on the first `scroll` (registered with **capture = true** so it also catches the reading column's nested scroll container — `scroll` does not bubble) and on `resize`, alongside the existing outside-click `mousedown` handler, all mounted together while `popover != null`. The `ShortFontAnchor.tsx` Radix variant has the same dismiss-on-scroll guard (Base UI's anchor tracking does **not** engage for the reading column's nested scroll container either). If you ever add live re-positioning instead, recompute `btnRect` on scroll — but dismissing is the chosen UX. Regression history: introduced when shortfont anchors were added to the reading-page prose in commit `d862b48`.
...
- sanskrit gatha
- sanskrit teeka
- hindi bhaavarth (`BhaavarthPanel`, target collection `gatha_teeka_bhaavarth_hindi`) — a `sanskrit_text` source block matches both the Sanskrit teeka and, via its `hindi_translation`, the भावार्थ panel; both highlight together as same-gatha siblings
