...
import type { KeywordPageSection } from '@/lib/types';
                    ...
                    {section.definitions.map((def) => (
                      <div key={def.definition_index} className="space-y-3">
                        {def.blocks.map((block, bi) => (
                          <div key={bi}>
                            {block.kind === 'sanskrit_text' || block.kind === 'prakrit_text' ? (
                              <div className="rounded border-l-4 border-cat-keyword bg-surface-muted p-3">
                                <p className="font-serif-hindi text-sm text-foreground">{block.text_devanagari}</p>
                              </div>
                            ) : (
