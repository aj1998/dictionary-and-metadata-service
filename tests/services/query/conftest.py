...
def make_mock_neo4j_topic_neighbors(
    neighbor_rows: list[dict] | None = None,
) -> object:
...
@pytest_asyncio.fixture
async def client_with_neo4j_topic_neighbors(request):  # type: ignore[return]
    """Client for the topic_neighbors endpoint.

    Pass (mongo_docs, neighbor_rows) via request.param.
    neighbor_rows must include anchor_nk field (returned by the topic_neighbors Cypher).
    """
    if not DATABASE_URL:
        pytest.skip("DATABASE_URL not set")
    ...
    params = getattr(request, "param", ([], []))
    mongo_docs: list[dict] = params[0] if len(params) > 0 else []
    neighbor_rows: list[dict] = params[1] if len(params) > 1 else []
    ...
    factory = async_sessionmaker(engine, expire_on_commit=False)
    mock_mongo = make_mock_mongo(mongo_docs)
    mock_neo4j = make_mock_neo4j_topic_neighbors(neighbor_rows)
